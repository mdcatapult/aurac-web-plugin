...
import { Entity, RecogniserEntities, SynonymData, SynonymText } from 'src/types';
import { BrowserService } from '../browser.service';
import { TestBrowserService } from '../test-browser.service';
import { HttpClientTestingModule } from '@angular/common/http/testing';

import { NerService, RecognisedEntity } from './ner.service';
...
describe('NerService', () => {
  ...
  it('should create an entity from an API response entity', () => {
    const recognisedEntity: RecognisedEntity = {
      entity: "Gene name",
      position: 5,
      xpath: "/html/*[1]/*[1]",
    ...
    const entity: Entity = {
      synonyms: new Map([["Gene name", { xpaths: ["/html/*[1]/*[1]"] }]]),
      identifiers: new Map([["resolvedEntity", "HGNC:8544"]]),
      metadata: {
  ...
  describe('setOrUpdateEntity', () => {
    ...
    beforeEach(() => {
      recogniserEntities = {
        ...
        entities: new Map<string, Entity>([["HGNC:8644", {
          synonyms: new Map<string, SynonymData>([["existing synonym", { xpaths: ["/html/*[1]"] }]])
        }]])
      }
    })
    ...
    it('should add new entity when entity is different', () => {
      const expectedRecogniserEntities = {
        ...
        entities: new Map<string, Entity>([
          ["HGNC:8644", { synonyms: new Map<string, SynonymData>([["existing synonym", { xpaths: ["/html/*[1]"] }]]) }],
          ["HGNC:8744", { synonyms: new Map<string, SynonymData>([["new synonym", { xpaths: ["/html/*[2]"] }]]) }]
        ])
      }
    ...
    it('should append a synonym when the entity is the same', () => {
      const expectedRecogniserEntities = {
        show: true,
        entities: new Map<string, Entity>([
          ["HGNC:8644", { synonyms: new Map<string, SynonymData>([["existing synonym", { xpaths: ["/html/*[1]"] }], ["new synonym", { xpaths: ["/html/*[2]"] }]]) }]
        ])
      }
    ...
    it('should append an xpath when the entity and synonym are the same', () => {
      const expectedRecogniserEntities = {
        show: true,
        entities: new Map<string, Entity>([
          ["HGNC:8644", { synonyms: new Map<string, SynonymData>([["existing synonym", { xpaths: ["/html/*[1]", "/html/*[2]"] }]]) }]
        ])
      }
      service["setOrUpdateEntity"](recogniserEntities, "HGNC:8644", {
  ...
  describe('transformAPIResponse', () => {
    ...
    it('transform a response from the ner api into a tab entities map', () => {
      const recognisedEntities: RecognisedEntity[] = [
        {
          entity: "entity1",
          position: 2,
          xpath: "/html/*[1]",
      ...
      const recogniserEntities: RecogniserEntities = {
        ...
        entities: new Map<string, Entity>([
          ["HGNC:8644", {
            synonyms: new Map<SynonymText, SynonymData>([
              ["entity1", { xpaths: ["/html/*[1]", "/html/*[7]"]}],
              ["entity2", { xpaths: ["/html/*[2]"] }]
            ]),
            identifiers: new Map<string,string>([["resolvedEntity", "HGNC:8644"]])
          }],
          ["HGNC:8633", {
            synonyms: new Map<SynonymText, SynonymData>([
              ["entity6", { xpaths: ["/html/*[6]"] }]
            ]),
            identifiers: new Map<string,string>([["resolvedEntity", "HGNC:8633"]])
          }],
          ["entity3", {
            synonyms: new Map<SynonymText, SynonymData>([
            ...
            identifiers: new Map<string,string>([["resolvedEntity", ""]])
          }],
          ["entity5", {
            synonyms: new Map<SynonymText, SynonymData>([
