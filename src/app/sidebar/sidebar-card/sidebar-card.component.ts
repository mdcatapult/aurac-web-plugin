...
export class SidebarCardComponent implements OnInit {
  ...
  constructor(private browserService: BrowserService, private sidebarDataService: SidebarDataService, private linksService: LinksService) {  }
  ...
  arrowClicked(direction: 'left' | 'right'): void {
    ...
    this.browserService.sendMessageToActiveTab({type: 'content_script_scroll_to_highlight', body: htmlTagIDs[this.scrollIndex]})
  }

  remove() {
    this.sidebarDataService.setCards(this.sidebarDataService.cards.filter(entity => entity.entityID !== this.card.entityID))
  }
